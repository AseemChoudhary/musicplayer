package com.templatemela.sangitguru.ringtone.designview;
...
public class CheapAMR extends CheapSoundFile {
        ...
        switch (frameType) {
            case 0:
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    int frac = (int)((log2 - log2) * 32768);
                    log2 -= 12;
                    int tmp = (int) (log2 * 49320);
                    tmp += ((frac * 24660) >> 15) * 2;
                    int quaEner = ((tmp * 8192) + 0x8000) >> 16;
                    int gcode0 = (385963008 +
                                    prevEner[0] * 5571 +
                                    prevEner[1] * 4751 +
                                    prevEner[2] * 2785 +
                                    prevEner[3] * 1556) >> 15;
                    ...
                    addFrame(mOffset, blockSize + 1, frameGainEstimate);
                }
            ...
            default:
                ...
                addFrame(mOffset, blockSize + 1, 1);
                break;
        }
    ...
    void addFrame(int offset, int frameSize, int gain) {
        ...
        if (mNumFrames == mMaxFrames) {
            ...
            int[] newOffsets = new int[mMaxFrames];
            int[] newLens = new int[mMaxFrames];
            mFrameGains = new int[mMaxFrames];
