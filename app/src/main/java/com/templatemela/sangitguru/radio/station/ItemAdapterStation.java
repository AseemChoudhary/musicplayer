package com.templatemela.sangitguru.radio.station;
...
import com.templatemela.sangitguru.radio.ActivityMain;
import com.templatemela.sangitguru.radio.MainRadioHelper;
import com.templatemela.sangitguru.radio.Utils;
import com.templatemela.sangitguru.radio.interfaces.IAdapterRefreshable;
...
import com.templatemela.sangitguru.radio.utils.RecyclerItemMoveAndSwipeHelper;
...
import com.templatemela.sangitguru.radio.utils.RecyclerItemSwipeHelper;
import com.templatemela.sangitguru.radio.utils.SwipeableViewHolder;
...
public class ItemAdapterStation
    ...
    public ItemAdapterStation(FragmentActivity fragmentActivity, int resourceId, StationsFilter.FilterType filterType) {
        ...
        stationImagePlaceholder = ContextCompat.getDrawable(fragmentActivity, R.drawable.song_images);

        MainRadioHelper radioDroidApp = (MainRadioHelper) fragmentActivity.getApplication();
    ...
    public void updateList(com.templatemela.sangitguru.radio.FragmentStarred refreshableList, List<DataRadioStation> stationsList) {
    ...
    public void onBindViewHolder(final StationViewHolder holder, int position) {
        ...
        Drawable flag = com.templatemela.sangitguru.radio.CountryFlagsLoader.getInstance().getFlag(activity, station.CountryCode);

        if (flag != null) {
            float k = flag.getMinimumWidth() / (float) flag.getMinimumHeight();
            float viewHeight = holder.textViewTitle.getTextSize();
            flag.setBounds(0, 0, (int) (k * viewHeight), (int) viewHeight);
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            holder.textViewTitle.setCompoundDrawablesRelative(flag, null, null, null);
        } else {
            holder.textViewTitle.setCompoundDrawables(flag, null, null, null);
        }
        ...
        if (isExpanded) {
            ...
            if (prefs.getBoolean("play_external", false)) {
                ...
                holder.buttonPlayInternalOrExternal.setOnClickListener(v -> Utils.playAndWarnIfMetered((MainRadioHelper) context.getApplicationContext(), station,
