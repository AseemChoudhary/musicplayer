package com.music.musicplayer.mp3player.radio.players.selector;
...
import com.music.musicplayer.mp3player.radio.players.mpd.MPDServerData;
import com.music.musicplayer.mp3player.radio.players.mpd.tasks.MPDChangeVolumeTask;
import com.music.musicplayer.mp3player.radio.players.mpd.tasks.MPDPauseTask;
import com.music.musicplayer.mp3player.radio.players.mpd.tasks.MPDResumeTask;
import com.music.musicplayer.mp3player.radio.players.mpd.tasks.MPDStopTask;
...
public class PlayerSelectorAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    protected PlayerSelectorAdapter(@NonNull Context context, @Nullable DataRadioStation stationToPlay) {
        ...
        MainRadioHelper mainRadioHelper =  (MainRadioHelper) context.getApplicationContext();

        this.context = context;
        this.inflater = LayoutInflater.from(context);

        this.mpdClient = mainRadioHelper.getMpdClient();
        ...
        if (mainRadioHelper.getCastHandler().isCastSessionAvailable()) {
            fixedViewsCount++;
        ...
        if (holder.getItemViewType() == PlayerType.RADIODROID.getValue()) {
            ...
            holder.btnPlay.setOnClickListener(view -> {
                if (PlayerServiceUtil.isPlaying()) {
                    ...
                    Utils.playAndWarnIfMetered((MainRadioHelper) context.getApplicationContext(), stationToPlay, PlayerType.RADIODROID,
                            () -> Utils.play((MainRadioHelper) context.getApplicationContext(), stationToPlay));
                }
            });
            ...
            holder.btnPlay.setOnClickListener(v -> Utils.playAndWarnIfMetered((MainRadioHelper) context.getApplicationContext(), stationToPlay,
