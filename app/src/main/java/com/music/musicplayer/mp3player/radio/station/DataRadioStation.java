...
import static com.music.musicplayer.mp3player.radio.Utils.resourceToUri;
...
public class DataRadioStation implements Parcelable {
	...
	public static final String RADIO_STATION_LOCAL_INFO_CHAGED = "com.music.musicplayer.mp3player.radio.radiostation.changed";
				...
				try {
					...
					for (int i = 0; i < jsonArray.length(); i++) {
						try {
							...
							Log.e(TAG, "DecodeJson() #2 "+exception);
						}
					}

				} catch (JSONException exception) {
					Log.e(TAG, "DecodeJson() #1 "+exception);
				}
			}
		}
		return aList;
		...
		if (result != null) {
			if (TextUtils.isGraphic(result)) {
				try {
					...
					Log.e(TAG, "DecodeJsonSingle() "+exception);
				}
			}
		}
		return null;
	...
	public static final Creator<DataRadioStation> CREATOR = new Creator<DataRadioStation>() {
		@Override
    ...
    public void prepareShortcut(Context ctx, ShortcutReadyListener cb) {
        Picasso.get()
                .load((!hasIcon() ? resourceToUri(ctx.getResources(), R.drawable.app_icon).toString() : IconUrl))
